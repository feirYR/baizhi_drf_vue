<template>
    <div class="cart">
        <headers/>
        <div class="cart-info">
            <h3 class="cart-top">购物车结算 <span>共{{bill_length}}门课程</span>&nbsp&nbsp&nbsp<span>订单号:{{order_number}}</span></h3>
            ...
            <div class="cart-item">
                <el-row v-for="course in courses " :key="course.id">
                    ...
                    <el-col :span="4" class="course-price">¥{{course.final_expire_price}}</el-col>
                    <el-col :span="4" class="course-price" v-if="course.final_expire_price !=course.price">
            ...
            <div class="calc">
                <el-row class="pay-row">
                    ...
                    <el-col :span="4" class="cart-pay"><span @click="course_order">支付宝支付</span></el-col>
                </el-row>
            </div>
        </div>
        <foot/>
        ...
        methods:{
            ...
            get_billCourse(){
                ...
                this.$axios({
                    ...
                    this.total_price = re.data['total_price']
                    this.bill_length = re.data.bill_length
                    this.$store.commit('add_goods',re.data.cart_length)
                    this.$message.success(re.data.message)
                }).catch(error=>{
                    this.$message.error('获取结算课程失败')
                })
            },
            course_order(){
                let token = this.check_user()
                this.$axios({
                    url:this.$settings.HOST+'order/courseOrder/',
                    method: 'post',
                    headers:{
                         'Authorization':'jwt ' + token
                    },
                    ...
                    this.order_number = re.data.order_number
                    this.$axios({
                        url:this.$settings.HOST+'payments/payLink/',
                        method:'get',
                        params:{
